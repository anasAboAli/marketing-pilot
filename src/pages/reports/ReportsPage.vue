...
const reports = [
  {
    name: "تقرير أداء الحملات",
    type: "أداء الحملات",
    date: "01/08/2026",
    status: "جاهز",
  },
  {
    name: "تقرير العملاء والمبيعات",
    type: "CRM",
    date: "05/08/2026",
    status: "جاهز",
  },
  {
    name: "ملخص الفريق والمهام",
    type: "الفريق",
    date: "07/08/2026",
    status: "جاهز",
  },
...
function exportCsv() {
  const rows = [
    ["اسم التقرير", "النوع", "التاريخ", "الحالة"],
    ...reports.map((report) => Object.values(report)),
  ];
  const csv = rows
    .map((row) =>
...
<template>
  <section id="margin" class="space-y-6">
    <div
    ...
    >
      <div>
        ...
        <p class="mt-2 text-slate-500">
          أنشئ تقارير واضحة وشاركها مع فريقك أو عملائك.
        </p>
      </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
      <div
        v-for="stat in [
          { label: 'إجمالي الإنفاق', value: '84,250 ر.س', change: '+18%' },
          { label: 'العملاء المحتملون', value: '1,284', change: '+24%' },
          { label: 'نسبة التحويل', value: '8.6%', change: '+3.2%' },
        ]"
        :key="stat.label"
      ...
      >
        ...
        <span
          class="mt-2 inline-flex rounded-full bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700"
          >{{ stat.change }} من الفترة السابقة</span
        >
      </div>
    </div>
    ...
    >
      ...
      >
        <h2 class="font-bold text-slate-900">التقارير المحفوظة</h2>
        <span class="text-sm text-slate-500">{{ selectedPeriod }}</span>
      </div>
      <div class="overflow-x-auto">
        <table class="w-full min-w-[620px] text-right text-sm">
          <thead class="bg-slate-50 text-slate-500">
            <tr>
              <th class="p-4 font-semibold">اسم التقرير</th>
          ...
          <tbody>
            ...
            >
              ...
              <td class="p-4 text-slate-500">{{ report.type }}</td>
              ...
              <td class="p-4">
                <button
                  class="viewBtn text-sm font-semibold text-blue-600 hover:text-blue-800"
                  @click="printReport"
                >
                  عرض التقرير
                </button>
